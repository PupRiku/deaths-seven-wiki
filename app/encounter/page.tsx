...
export default function EncounterPage() {
  ...
  function addCombatant(c?: Partial<Combatant>) {
    ...
    setNewCombatant({ name: '', initiative: '', hp: '', ac: '', isPlayer: false, isEnemy: true, notes: '' })
    setShowAddForm(false)
  }

  function addParty() {
    PARTY.forEach((p) => addCombatant({ ...p, maxHp: p.hp, initiative: Math.floor(Math.random() * 20) + 1, conditions: [], isEnemy: false }))
  }

  function addPreset(preset: typeof PRESET_ENEMIES[0]) {
    addCombatant({ name: preset.name, hp: preset.hp, maxHp: preset.hp, ac: preset.ac, initiative: Math.floor(Math.random() * 20) + 1, isPlayer: false, isEnemy: true, conditions: [], notes: `CR ${preset.cr}` })
  ...
  function setHP(id: string, val: number) {
    setCombatants((prev) => prev.map((c) => c.id === id ? { ...c, hp: Math.max(0, Math.min(c.maxHp, val)) } : c))
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '0.75rem 1.25rem', borderBottom: '0.5px solid var(--border)', background: 'var(--bg-base)', display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          {!isActive ? (
            <button className="btn btn-primary" onClick={() => setIsActive(true)} disabled={combatants.length === 0}>▶ Start Combat</button>
          ) : (
            <>
              <button className="btn btn-ghost" onClick={prevTurn}>← Prev</button>
              <button className="btn btn-primary" onClick={nextTurn}>Next →</button>
            </>
          )}
          <button className="btn btn-ghost" onClick={() => setShowPresets(!showPresets)}>+ Enemy Preset</button>
          <button className="btn btn-ghost" onClick={addParty}>+ Party</button>
          <button className="btn btn-ghost" onClick={() => setShowAddForm(!showAddForm)}>+ Custom</button>
            ...
            return (
              <div key={c.id} className={`combatant-row ${isCurrent ? 'active' : ''} ${c.isPlayer ? 'player' : ''} ${c.isEnemy ? 'enemy' : ''}`}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', width: '100%' }}>
                  ...
                  {/* Remove */}
                  <button onClick={() => removeCombatant(c.id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.9rem', padding: '0 0.25rem' }}>×</button>
                </div>

                {/* HP bar */}
                <div className="hp-bar" style={{ width: '100%' }}>
                  <div className={`hp-bar-fill ${hpCls}`} style={{ width: `${hpPct}%` }} />
                </div>
